...
const loader = require(***REMOVED***../util/loader***REMOVED***);
...
function loadOpus(refresh = false) {
  ...
  Opus = loader.require([
    [***REMOVED***@discordjs/opus***REMOVED***, opus => ({ Encoder: opus.OpusEncoder })],
    [***REMOVED***node-opus***REMOVED***, opus => ({ Encoder: opus.OpusEncoder })],
    [***REMOVED***opusscript***REMOVED***, opus => ({ Encoder: opus })],
  ]);
  return Opus;
}

const charCode = x => x.charCodeAt(0);
const OPUS_HEAD = Buffer.from([...***REMOVED***OpusHead***REMOVED***].map(charCode));
const OPUS_TAGS = Buffer.from([...***REMOVED***OpusTags***REMOVED***].map(charCode));

// frame size = (channels * rate * frame_duration) / 1000

/**
 * Takes a stream of Opus data and outputs a stream of PCM data, or the inverse.
 * **You shouldn***REMOVED***t directly instantiate this class, see opus.Encoder and opus.Decoder instead!**
 * @memberof opus
...
class OpusStream extends Transform {
  ...
  constructor(options = {}) {
    if (!loadOpus().Encoder) {
      throw Error(***REMOVED***Could not find an Opus module! Please install @discordjs/opus, node-opus, or opusscript.***REMOVED***);
    }
    super(Object.assign({ readableObjectMode: true }, options));
    if (Opus.name === ***REMOVED***opusscript***REMOVED***) {
  ...
  _decode(buffer) {
    return this.encoder.decode(buffer, Opus.name === ***REMOVED***opusscript***REMOVED*** ? null : this._options.frameSize);
  }
  ...
  _final(cb) {
    if (Opus.name === ***REMOVED***opusscript***REMOVED*** && this.encoder) this.encoder.delete();
    cb();
  }
}
...
class Decoder extends OpusStream {
  _transform(chunk, encoding, done) {
    ...
    if (signature.equals(OPUS_HEAD)) {
      this.emit(***REMOVED***format***REMOVED***, {
    ...
    if (signature.equals(OPUS_TAGS)) {
      this.emit(***REMOVED***tags***REMOVED***, chunk);
      return done();
    }
