...
const OGGS_HEADER = Buffer.from([...***REMOVED***OggS***REMOVED***].map(charCode));
...
class OggDemuxer extends Transform {
  ...
  _readPage(chunk) {
    ...
    for (const size of sizes) {
      ...
      if (this._head) {
        if (header.equals(OPUS_TAGS)) this.emit(***REMOVED***tags***REMOVED***, segment);
        else if (this._bitstream === bitstream) this.push(segment);
      } else if (header.equals(OPUS_HEAD)) {
        this.emit(***REMOVED***head***REMOVED***, segment);
        this._head = segment;
        this._bitstream = bitstream;
      } else {
        this.emit(***REMOVED***unknownSegment***REMOVED***, segment);
      }
      start += size;
    }
