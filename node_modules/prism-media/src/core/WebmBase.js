const { Transform } = require(***REMOVED***stream***REMOVED***);

/**
 * Base class for WebmOpusDemuxer and WebmVorbisDemuxer.
 * **You shouldn***REMOVED***t directly instantiate this class, use the opus.WebmDemuxer and vorbis.WebmDemuxer
...
class WebmBaseDemuxer extends Transform {
   ...
   * Returns the TOO_SHORT symbol if the data wasn***REMOVED***t big enough to facilitate the request.
   */
  ...
  _readTag(chunk, offset) {
    ...
    const ebmlID = idData.id.toString(***REMOVED***hex***REMOVED***);
    if (!this._ebmlFound) {
      if (ebmlID === ***REMOVED***1a45dfa3***REMOVED***) this._ebmlFound = true;
      else throw Error(***REMOVED***Did not find the EBML tag at the start of the stream***REMOVED***);
    }
    ...
    // If this tag isn***REMOVED***t useful, tell the stream to stop processing data until the tag ends
    if (typeof TAGS[ebmlID] === ***REMOVED***undefined***REMOVED***) {
    ...
    if (!this._track) {
      if (ebmlID === ***REMOVED***ae***REMOVED***) this._incompleteTrack = {};
      if (ebmlID === ***REMOVED***d7***REMOVED***) this._incompleteTrack.number = data[0];
      if (ebmlID === ***REMOVED***83***REMOVED***) this._incompleteTrack.type = data[0];
      if (this._incompleteTrack.type === 2 && typeof this._incompleteTrack.number !== ***REMOVED***undefined***REMOVED***) {
        this._track = this._incompleteTrack;
      }
    }
    if (ebmlID === ***REMOVED***63a2***REMOVED***) {
      this._checkHead(data);
    } else if (ebmlID === ***REMOVED***a3***REMOVED***) {
      if (!this._track) throw Error(***REMOVED***No audio track in this webm!***REMOVED***);
 ...
 * @type {Symbol}
 */
const TOO_SHORT = WebmBaseDemuxer.TOO_SHORT = Symbol(***REMOVED***TOO_SHORT***REMOVED***);
...
const TAGS = WebmBaseDemuxer.TAGS = { // value is true if the element has children
  ***REMOVED***1a45dfa3***REMOVED***: true, // EBML
  ***REMOVED***18538067***REMOVED***: true, // Segment
  ***REMOVED***1f43b675***REMOVED***: true, // Cluster
  ***REMOVED***1654ae6b***REMOVED***: true, // Tracks
  ***REMOVED***ae***REMOVED***: true, // TrackEntry
  ***REMOVED***d7***REMOVED***: false, // TrackNumber
  ***REMOVED***83***REMOVED***: false, // TrackType
  ***REMOVED***a3***REMOVED***: false, // SimpleBlock
  ***REMOVED***63a2***REMOVED***: false,
};
