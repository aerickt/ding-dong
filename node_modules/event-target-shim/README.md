...
- `options` is a boolean or an object `{ capture?: boolean, passive?: boolean, once?: boolean }`. If this is a boolean, it***REMOVED***s same meaning as `{ capture: options }`.
...
- `options` is a boolean or an object `{ capture?: boolean }`. If this is a boolean, it***REMOVED***s same meaning as `{ capture: options }`.
    - `capture` is the flag to register the event listener for capture phase.
...
- `proto` is an object (assuming it***REMOVED***s a prototype object). This function defines a getter/setter pair for the event attribute.
...
// Define `foo.onhello` property***REMOVED***s implementation.
defineEventAttribute(Foo.prototype, "hello")
...
Unfortunately, both `FooEvents` and `FooEventAttributes` are needed because TypeScript doesn***REMOVED***t allow the mutation of string literal types. If TypeScript allowed us to compute `"onhello"` from `"hello"` in types, `FooEventAttributes` will be optional.
