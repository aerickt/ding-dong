...
const { Writable } = require(***REMOVED***stream***REMOVED***);
const secretbox = require(***REMOVED***../util/Secretbox***REMOVED***);
const Silence = require(***REMOVED***../util/Silence***REMOVED***);
const VolumeInterface = require(***REMOVED***../util/VolumeInterface***REMOVED***);
 ...
 *   const dispatcher = connection.play(***REMOVED***/home/hydrabolt/audio.mp3***REMOVED***);
 * });
 * ```
...
class StreamDispatcher extends Writable {
  constructor(player, { seek = 0, volume = 1, fec, plp, bitrate = 96, highWaterMark = 12 } = {}, streams) {
    ...
    this.on(***REMOVED***finish***REMOVED***, () => {
      this._cleanup();
      this._setSpeaking(0);
    });
    ...
    if (typeof fec !== ***REMOVED***undefined***REMOVED***) this.setFEC(fec);
    if (typeof plp !== ***REMOVED***undefined***REMOVED***) this.setPLP(plp);
    ...
    const streamError = (type, err) => {
      ...
      if (type && err) {
        ...
        this.emit(this.player.dispatcher === this ? ***REMOVED***error***REMOVED*** : ***REMOVED***debug***REMOVED***, err);
      }
      this.destroy();
    };

    this.on(***REMOVED***error***REMOVED***, () => streamError());
    if (this.streams.input) this.streams.input.on(***REMOVED***error***REMOVED***, err => streamError(***REMOVED***input***REMOVED***, err));
    if (this.streams.ffmpeg) this.streams.ffmpeg.on(***REMOVED***error***REMOVED***, err => streamError(***REMOVED***ffmpeg***REMOVED***, err));
    if (this.streams.opus) this.streams.opus.on(***REMOVED***error***REMOVED***, err => streamError(***REMOVED***opus***REMOVED***, err));
    if (this.streams.volume) this.streams.volume.on(***REMOVED***error***REMOVED***, err => streamError(***REMOVED***volume***REMOVED***, err));
  }
  ...
  _write(chunk, enc, done) {
    if (!this.startTime) {
       ...
       */
      this.emit(***REMOVED***start***REMOVED***);
  ...
  resume() {
    ...
    this.pausedSince = null;
    if (typeof this._writeCallback === ***REMOVED***function***REMOVED***) this._writeCallback();
  }
  ...
  setBitrate(value) {
    ...
    const bitrate = value === ***REMOVED***auto***REMOVED*** ? this.player.voiceConnection.channel.bitrate : value;
  ...
  _encrypt(buffer) {
    ...
    if (mode === ***REMOVED***xsalsa20_poly1305_lite***REMOVED***) {
      this._nonce++;
    ...
    } else if (mode === ***REMOVED***xsalsa20_poly1305_suffix***REMOVED***) {
  ...
  _sendPacket(packet) {
    ...
    if (!this.player.voiceConnection.sockets.udp) {
      this.emit(***REMOVED***debug***REMOVED***, ***REMOVED***Failed to send a packet - no UDP socket***REMOVED***);
      return;
    }
    this.player.voiceConnection.sockets.udp.send(packet).catch(e => {
      this._setSpeaking(0);
      this.emit(***REMOVED***debug***REMOVED***, `Failed to send a packet - ${e}`);
    });
  }

  _setSpeaking(value) {
    if (typeof this.player.voiceConnection !== ***REMOVED***undefined***REMOVED***) {
     ...
     */
    this.emit(***REMOVED***speaking***REMOVED***, value);
  }
  ...
  setVolume(value) {
     ...
     */
    this.emit(***REMOVED***volumeChange***REMOVED***, this.volume, value);
