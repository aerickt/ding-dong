...
(something users of TweetNaCl shouldn***REMOVED***t care about).
...
> anything in the real world is essentially zero luckily, but it***REMOVED***s good to have
> it fixed."
...
* **Breaking change!** All functions from `nacl.util` have been removed. These
  ...
  or
      ...
      var nacl = require(***REMOVED***tweetnacl***REMOVED***);
      nacl.util = require(***REMOVED***tweetnacl-util***REMOVED***);
...
* Implement `nacl.sign.keyPair.fromSeed` to enable creation of sign key pairs
  ...
  [libsodium***REMOVED***s](http://doc.libsodium.org/public-key_cryptography/public-key_signatures.html)
...
* **Signature API breaking change!** `nacl.sign` and `nacl.sign.open` now deal
 ...
 message and "detached" signature. This was unlike NaCl***REMOVED***s API, which dealt with
 ...
 (Note that it***REMOVED***s `verify`, not `open`, and it returns a boolean value, unlike
...
* Fast version: increased theoretical message size limit from 2^32-1 to 2^52
  ...
  indexes, and most implementations won***REMOVED***t allocate more than a gigabyte or so.
  (Obviously, there are no tests for the correctness of implementation.) Also,
  it***REMOVED***s not recommended to use messages that large without splitting them into
