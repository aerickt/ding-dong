...
- Fix: incorrect `Content-Length` was returned for stream body in 2.5.0 release; note that `node-fetch` doesn***REMOVED***t calculate content length for stream body.
...
- Fix: `Blob` import rule for node < 10, as `Readable` isn***REMOVED***t a named export.

## v2.4.0

- Enhance: added `Brotli` compression support (using node***REMOVED***s zlib).
...
- Fix: update `browser.js` to support react-native environment, where `self` isn***REMOVED***t available globally.

## v2.2.1

- Fix: `compress` flag shouldn***REMOVED***t overwrite existing `Accept-Encoding` header.
- Fix: multiple `import` rules, where `PassThrough` etc. doesn***REMOVED***t have a named export when using node <10 and `--exerimental-modules` flag.
- Other: Better README.
...
- Enhance: Support Node.js***REMOVED*** `--experimental-modules` mode; deprecate `.es.js` file
...
- Major: `require(***REMOVED***node-fetch/lib/response***REMOVED***)` etc. is now unsupported; use `require(***REMOVED***node-fetch***REMOVED***).Response` or ES6 module imports
...
- Major: overwrite user***REMOVED***s `Content-Length` if we can be sure our information is correct (per spec)
- Fix: errors in a response are caught before the body is accessed
- Fix: support WHATWG URL objects, created by `whatwg-url` package or `require(***REMOVED***url***REMOVED***).URL` in Node.js 7+
...
- Enhance: minor document update
- Fix: HEAD request doesn***REMOVED***t need decompression, as body is empty
...
- Enhance: allow custom host header to be set (server-side only feature, as it***REMOVED***s a forbidden header on client-side)
